...
const Syarat: React.FC = () => {
  const [userSyarat, setUserSyarat] = useState<boolean>(false);
  const [narasumberSyarat, setNarasumberSyarat] = useState<boolean>(false);

  const toggleUserSyarat = () => {
    setUserSyarat((prevState) => !prevState);
  };

  const toggleNarasumberSyarat = () => {
    setNarasumberSyarat((prevState) => !prevState);
  };
  return (
    <div className="syarat-wrapper">
      <div className="syarat-option" onClick={() => toggleUserSyarat()}>
        <span>User</span>
        <img
          src={arrow_right_black}
          alt="arrow black"
          className={userSyarat ? "syarat-active" : "syarat-deactive"}
        />
      </div>
      {userSyarat ? (
        <div className="syarat-user">
      ...
      <div className="syarat-option" onClick={() => toggleNarasumberSyarat()}>
        <span>Narasumber</span>
        <img
          src={arrow_right_black}
          alt="arrow black"
          className={narasumberSyarat ? "syarat-active" : "syarat-deactive"}
        />
      </div>
      {narasumberSyarat ? (
        <div className="syarat-narasumber">
          <span>
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Ipsa quia
            ea est impedit animi architecto natus quo dolorem error sed ratione
            ullam inventore, fugiat facilis eligendi enim omnis earum expedita.
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Quae esse
            optio, cupiditate voluptates ratione ullam possimus doloremque quos
            earum illum! Placeat veritatis, eveniet dicta vero tempora dolor
            sint ipsum praesentium? Lorem, ipsum dolor sit amet consectetur
            adipisicing elit. Vel, iusto deleniti? Tempora magnam inventore sint
            fugiat recusandae nisi rem ipsam enim minus dignissimos adipisci,
            est corporis commodi hic ex dolores. Lorem ipsum dolor sit, amet
            consectetur adipisicing elit. Quos aliquid debitis voluptate dicta,
            exercitationem, architecto aperiam, corporis laboriosam maxime cum
            in. Quos deleniti facilis odit harum eos, corporis repellendus rem.
          </span>
        </div>
      ) : (
        <></>
